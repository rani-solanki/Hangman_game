...
def get_available_letters(letters_guessed):
    ...
    '''
    import string
    new_letter= string.ascii_lowercase
    print(new_letter)
...
def hangman(secret_word):
    ...
    * Har round ke baar, user ko uska guess kiya hua partial word
    ...
    user=input("enter the level""\n""a for essy""\n""b is medium level""\n""c for hard level")
    if user=="b":
        total_lives = remaining_lives =6
        image_selection=[1,2,3,4,5,6,7]
    elif user=="c":
        total_lives = remaining_lives=4
        image_selection=[1,3,4,5]
    elif user!="a":
        print ("your choice is invalid""\n""it will starting from essy level")
    while (remaining_lives>0):
        ...
        letter = guess.lower()
        if guess == "hint":
            print("your secret world:" + get_hint(secret_word, letters_guessed))
